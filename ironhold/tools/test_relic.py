...
with sync_playwright() as p:
    ...
    for name,label in [(None,'fresh, Goblin Village'),('asc_1_9.txt','realm 9, Giant Hills')]:
        ...
        r=pg.evaluate("""()=>{if(S.asc.realm===0){S.area=1;S.cleared[0]=true;}const c0=(S.chests||0)+(S.chests2||0);S.last=Date.now()-2*3600e3/coachSpeed();const d=offline(true,true);G.away=null;return {chests:(S.chests||0)+(S.chests2||0)-c0,h:d.t/3600,kills:d.kills,area:AREAS[S.area].n}}""")
        print(label,r)
        chk('about a chest a quarter-hour in '+label,4<=r['chests']/r['h']<=12,'%.1f per hour'%(r['chests']/r['h']))
