...
def kind(key):
    ...
    if key.startswith('ui_cabinet_') or key in ('ui_cabinet', 'ui_panel', 'ui_button', 'ui_spin_button', 'ui_stop_button', 'ui_logo', 'ui_slot_frame', 'ui_ooze_button', 'ui_death_button'):
...
OUT_SKINS = os.path.join(os.path.dirname(OUT), 'art-skins.js')
SKIN_KEY = re.compile(r'^hero_(knight|ranger|gambler|brute|duelist|hexpriest)_[a-z]+_(idle|walk|attack|hurt)$')
...
def main():
    ...
    # hero skins (hero_<hero>_<skin>_<pose>) go to their own file, fetched after the game is up,
    # so 192 skin sprites never slow the first load
    skins = {k: v for k, v in art.items() if SKIN_KEY.match(k)}
    for k in skins: del art[k]
    with open(OUT_SKINS, 'w') as fh:
        fh.write('// GENERATED by pack-art.py from art-src/ — do not edit by hand. Hero skin sprites, loaded after art.js.\n')
        fh.write('window.LD_ART_SKINS = ' + json.dumps(skins, separators=(',', ':')).replace('","', '",\n"') + ';\n')
    ...
    print(f'{len(art)} images -> art.js, {len(skins)} hero skin sprites -> art-skins.js, {total/1024/1024:.2f}MB in all')
